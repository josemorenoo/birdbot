...
class PostMaker:
    def __init__(self, sts_secrets: Dict[str, str]):
    ...
    def generate_tweet_text(self, report_date, metric, timeframe="DAILY"):
        summary_report_dict = report_util.get_summary_report(report_date, timeframe)
        token_hashtags = " ".join(
    ...
    def post_loc_chart(self, report_date, timeframe="DAILY"):
        ...
        top_loc_img_path = f'reports/{timeframe.lower()}/{report_date.strftime("%Y-%m-%d")}/{GRAPH_NAMES["LOC_AND_EXT"]}'
    ...
    def post_authors_chart(self, report_date, timeframe="DAILY"):
        ...
        top_distinct_authors_img_path = f'reports/{timeframe.lower()}/{report_date.strftime("%Y-%m-%d")}/{GRAPH_NAMES["AUTHORS_AND_EXT"]}'
    ...
    def post_top_commits_chart(self, report_date, timeframe="DAILY"):
        ...
        top_commits_img_path = f'reports/{timeframe.lower()}/{report_date.strftime("%Y-%m-%d")}/{GRAPH_NAMES["COMMITS_AND_EXT"]}'
